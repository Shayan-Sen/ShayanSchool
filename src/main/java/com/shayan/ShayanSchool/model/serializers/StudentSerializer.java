...
public class StudentSerializer extends JsonSerializer<Student> {
    ...
    public void serialize(Student student, JsonGenerator gen, SerializerProvider serializer) throws IOException {
        ...
        gen.writeStringField("date_of_enrollment", student.getDateOfEnrollment() != null ? student.getDateOfEnrollment().toString() : null);
        gen.writeFieldName("class");
        gen.writeStartObject();
        gen.writeStringField("id", student.getClassRoom() == null ? "No class assigned yet" : student.getClassRoom().getId().toString());
        gen.writeStringField("name", student.getClassRoom() == null ? "No class assigned yet" : student.getClassRoom().getName());
        gen.writeEndObject();
        gen.writeEndObject();
